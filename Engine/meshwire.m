function strcoil = meshwire(Pcenter, x, y, direction, sk, phi)
...
%   Copyright SNM/JGF 2022/2023
    ...
    if norm(direction)>0
        angle = correction(direction, UnitPathVector(1, :), Pcross);
    ...
    for m = 1:Steps
        ...
        if norm(direction)>0
            ...
            angle(m)           = correction(direction, UnitPathVector(m+1, :), ptemp-ptempmean);
            ptemp           = meshrotate_axis(ptemp-ptempmean, UnitPathVector(m, :), angle(m)) + ptempmean;
        end
        %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
